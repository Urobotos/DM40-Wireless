"""Format values for the graph (scale, MIN, MAX)."""
...
def format_graph_value(norm_value: float, unit: str, decimals: int) -> str:
    """Compact notation as on the DM40 (e.g. ``-0.0435V``, ``0.000MΩ``)."""
...
def format_graph_scale_label(norm_value: float, unit: str, decimals: int) -> str:
    """Left-side graph scale – sign column for top/mid/bot alignment."""
    mul = axis_mul_for_unit(unit)
    disp = norm_value / mul
    sign = "-" if disp < 0 else " "
